<%@ Page Title="Halls" Language="C#" MasterPageFile="~/pages/Admin.Master" AutoEventWireup="true" CodeFile="halls.aspx.cs" Inherits="KumariCinema.Admin.halls" %>
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <button type="button" class="btn btn-sm btn-warning me-1" data-bs-toggle="modal" data-bs-target="#editHallModal"
                                        data-id='<%# Eval("HallId") %>'
                                        data-name='<%# System.Web.HttpUtility.HtmlAttributeEncode(Eval("HallName").ToString()) %>'
                                        data-capacity='<%# Eval("Capacity") %>'
                                        data-screen='<%# Eval("ScreenType") %>'
                                        data-theater='<%# Eval("TheaterId") %>'
                                        onclick="editHall(this)">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <asp:RequiredFieldValidator ControlToValidate="hallNameInput" ErrorMessage="Hall name is required" CssClass="text-danger" Display="Dynamic" runat="server" ValidationGroup="addGroup"></asp:RequiredFieldValidator>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Capacity</label>
                        <asp:TextBox ID="capacityInput" runat="server" CssClass="form-control" TextMode="Number" placeholder="120"></asp:TextBox>
                        <asp:RequiredFieldValidator ControlToValidate="capacityInput" ErrorMessage="Capacity is required" CssClass="text-danger" Display="Dynamic" runat="server" ValidationGroup="addGroup"></asp:RequiredFieldValidator>
                    </div>
                    <div class="mb-3">
                        ...
                        <asp:RequiredFieldValidator ControlToValidate="screenTypeDropdown" ErrorMessage="Screen type is required" CssClass="text-danger" Display="Dynamic" runat="server" ValidationGroup="addGroup"></asp:RequiredFieldValidator>
                    </div>
                    <div class="mb-3">
                ...
                <div class="modal-footer">
                    ...
                    <asp:Button ID="saveHallButton" runat="server" Text="Save Hall" CssClass="btn btn-primary-custom" OnClick="SaveHall_Click" ValidationGroup="addGroup" OnClientClick="setModalState('add'); return true;" />
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="editHallModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <asp:Button ID="updateHallButton" runat="server" Text="Update Hall" CssClass="btn btn-primary-custom" OnClick="UpdateHall_Click" OnClientClick="setModalState('edit'); return true;" />
                </div>
            </div>
        </div>
    </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="scripts" runat="server">
    <script>
        function editHall(btn) {
            const d = btn.dataset;
            document.getElementById('<%= editHallIdInput.ClientID %>').value = d.id;
            document.getElementById('<%= editHallNameInput.ClientID %>').value = d.name;
            document.getElementById('<%= editCapacityInput.ClientID %>').value = d.capacity;
            document.getElementById('<%= editScreenTypeDropdown.ClientID %>').value = d.screen;
            document.getElementById('<%= editTheaterDropdown.ClientID %>').value = d.theater;
            document.getElementById('<%= editHallIdField.ClientID %>').value = d.id;
        }

        function deleteHall(hallId) {
            showConfirm('Are you sure you want to delete this hall? This will also delete all associated shows and bookings.', function () {
        ...
        window.addEventListener('load', function() {
            ...
            if (modalState === 'add') {
                const addModal = new bootstrap.Modal(document.getElementById('addHallModal'));
                addModal.show();
